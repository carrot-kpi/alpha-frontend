...
import { ManualRealityV100Data } from '@carrot-kpi/decoder'
...
interface ManualRealityV100OracleDataProps {
  data: ManualRealityV100Data
}

export const ManualRealityV100OracleAction = ({ data }: ManualRealityV100OracleDataProps) => {
  const { chainId, account } = useActiveWeb3React()
  const questionId = useMemo(() => {
    return data.questionId
  }, [data.questionId])
  const { loading: loadingRealityQuestionData, data: questionData } = useRealityQuestion(questionId)
  ...
  const binary = useMemo(() => data.templateId.isZero(), [data.templateId])
  ...
  if (data.openingTimestamp > Math.floor(Date.now() / 1000)) {
    return (
      <>
        The Reality.eth question will become answerable on{' '}
        {DateTime.fromSeconds(data.openingTimestamp).toLocaleString(DateTime.DATETIME_MED)}
      </>
    )
  }
